/******************************
  ...
  Murphi Last Compiled date: "Apr  2 2025"
 ******************************/
...
class mu_1_MessageType: public mu__byte
{
 public:
  ...
  mu_1_MessageType (char *name, int os): mu__byte(7, 17, 4, name, os) {};
  mu_1_MessageType (void): mu__byte(7, 17, 4) {};
  mu_1_MessageType (int val): mu__byte(7, 17, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1_MessageType::values[] = {"ReadReq","ReadAck","ReadExReq","ReadExAck","WBReq","WBAck","RecallReq","InvReq","DowngradeReq","DowngradeAck","AutoDowngradeReq",NULL };
...
class mu_1__type_0: public mu__byte
{
 public:
  ...
  mu_1__type_0 (char *name, int os): mu__byte(18, 22, 3, name, os) {};
  mu_1__type_0 (void): mu__byte(18, 22, 3) {};
  mu_1__type_0 (int val): mu__byte(18, 22, 3, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1__type_0::values[] = {"H_Modified","H_Invalid","H_Shared","HT_Pending","HT_S2MPending",NULL };
...
class mu_1__type_2: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1__type_2::values[ int(val) - 23] );
    else return ( s << "Undefined" );
  };

  mu_1__type_2 (char *name, int os): mu__byte(23, 30, 4, name, os) {};
  mu_1__type_2 (void): mu__byte(23, 30, 4) {};
  mu_1__type_2 (int val): mu__byte(23, 30, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1__type_2::values[] = {"P_Modified","P_Invalid","P_Shared","PT_I2SPending","PT_I2MPending","PT_S2MPending","PT_WritebackPending","PT_M2SPending",NULL };
...
void mu_1__type_7::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"HomeType", i * 96 + os);i++;
array[i].set_self_ar(n,"Proc_1", i * 96 + os);i++;
array[i].set_self_ar(n,"Proc_2", i * 96 + os);i++;
array[i].set_self_ar(n,"Proc_3", i * 96 + os);i++;
}
...
const int mu_ReadExReq = 9;
const int mu_ReadExAck = 10;
const int mu_WBReq = 11;
const int mu_WBAck = 12;
const int mu_RecallReq = 13;
const int mu_InvReq = 14;
const int mu_DowngradeReq = 15;
const int mu_DowngradeAck = 16;
const int mu_AutoDowngradeReq = 17;
const int mu_H_Modified = 18;
const int mu_H_Invalid = 19;
const int mu_H_Shared = 20;
const int mu_HT_Pending = 21;
const int mu_HT_S2MPending = 22;
const int mu_P_Modified = 23;
const int mu_P_Invalid = 24;
const int mu_P_Shared = 25;
const int mu_PT_I2SPending = 26;
const int mu_PT_I2MPending = 27;
const int mu_PT_S2MPending = 28;
const int mu_PT_WritebackPending = 29;
const int mu_PT_M2SPending = 30;
...
/*** Variable declaration ***/
mu_1__type_7 mu_InBox("InBox",760);

/*** Variable declaration ***/
mu_0_boolean mu_msg_processed("msg_processed",1144);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",1152);
...
if ( (mu_n) != (mu_rqst) )
{
mu_Send ( mu_InvReq, mu_n, (int)mu_HomeType, mu_VC2, mu_1_Value_undefined_var );
}
}
};
};
...
void mu_SendAckToSharers(const mu_1_MessageType& mu_mtype, const mu_1_Value& mu_val, const mu_1_Node& mu_rqst)
{
{
for(int mu_n = 1; mu_n <= 6; mu_n++)
  if (( ( mu_n >= 1 ) && ( mu_n <= 3 ) )|| ( ( mu_n >= 6 ) && ( mu_n <= 6 ) )) {
bool mu__boolexpr15;
  if (!((mu_n>=1 && mu_n<=3))) mu__boolexpr15 = FALSE ;
  else {
...
/*** end multisetcount 4 declaration ***/
  mu__boolexpr15 = ((mu__intexpr16) != (0)) ; 
}
if ( mu__boolexpr15 )
{
if ( (mu_n) != (mu_rqst) )
{
mu_Send ( mu_mtype, mu_n, (int)mu_HomeType, mu_VC1, mu_val );
}
}
};
};
...
case mu_H_Invalid:
if ( !((mu_HomeNode.mu_owner.isundefined()) == (mu_true)) ) Error.Error("Assertion failed: msi_opt.m, line 183.");
if ( !((mu_cnt) == (0)) ) Error.Error("Assertion failed: HomeNode has sharers, but line is Invalid");
...
break;
case mu_ReadExReq:
if ( !((mu_cnt) == (0)) ) Error.Error("Assertion failed: ReadExReq with sharers");
mu_HomeNode.mu_state = mu_H_Modified;
mu_HomeNode.mu_owner = mu_msg.mu_src;
mu_Send ( mu_ReadExAck, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val );
break;
default:
...
break;
case mu_H_Modified:
if ( !((mu_HomeNode.mu_owner.isundefined()) == (mu_false)) ) Error.Error("Assertion failed: msi_opt.m, line 204.");
if ( !((mu_cnt) == (0)) ) Error.Error("Assertion failed: HomeNode has sharers, but line is Modified");
switch ((int) mu_msg.mu_mtype) {
case mu_ReadReq:
if ( !((mu_msg.mu_src) != (mu_HomeNode.mu_owner)) ) Error.Error("Assertion failed: ReadReq from owner");
mu_HomeNode.mu_state = mu_HT_Pending;
mu_Send ( mu_DowngradeReq, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC0, mu_1_Value_undefined_var );
mu_AddToSharersList ( mu_msg.mu_src );
mu_AddToSharersList ( mu_HomeNode.mu_owner );
mu_HomeNode.mu_owner.undefine();
break;
case mu_ReadExReq:
if ( !((mu_msg.mu_src) != (mu_HomeNode.mu_owner)) ) Error.Error("Assertion failed: ReadExReq from owner");
mu_HomeNode.mu_state = mu_HT_Pending;
mu_Send ( mu_RecallReq, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC0, mu_1_Value_undefined_var );
mu_HomeNode.mu_owner = mu_msg.mu_src;
break;
case mu_WBReq:
if ( !((mu_msg.mu_src) == (mu_HomeNode.mu_owner)) ) Error.Error("Assertion failed: Modified state: Writeback from non-owner");
...
mu_HomeNode.mu_owner.undefine();
break;
case mu_AutoDowngradeReq:
if ( !((mu_msg.mu_src) == (mu_HomeNode.mu_owner)) ) Error.Error("Assertion failed: Modified state: AutoDowngradeReq from non-owner");
mu_HomeNode.mu_state = mu_H_Shared;
mu_HomeNode.mu_val = mu_msg.mu_val;
mu_AddToSharersList ( mu_msg.mu_src );
mu_Send ( mu_ReadAck, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val );
...
break;
case mu_H_Shared:
if ( !((mu_HomeNode.mu_owner.isundefined()) == (mu_true)) ) Error.Error("Assertion failed: msi_opt.m, line 245.");
if ( !((mu_cnt) > (0)) ) Error.Error("Assertion failed: HomeNode has no sharers, but line is Shared");
switch ((int) mu_msg.mu_mtype) {
case mu_ReadReq:
if ( !((mu_IsSharer( mu_msg.mu_src )) == (mu_false)) ) Error.Error("Assertion failed: ReadReq from sharer");
mu_Send ( mu_ReadAck, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val );
mu_AddToSharersList ( mu_msg.mu_src );
break;
case mu_ReadExReq:
mu_HomeNode.mu_state = mu_HT_S2MPending;
...
break;
case mu_WBReq:
if ( !((mu_IsSharer( mu_msg.mu_src )) == (mu_true)) ) Error.Error("Assertion failed: Writeback from non-sharer");
mu_RemoveFromSharersList ( mu_msg.mu_src );
mu_Send ( mu_WBAck, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_1_Value_undefined_var );
if ( !((mu_IsSharer( mu_msg.mu_src )) == (mu_false)) ) Error.Error("Assertion failed: Sharer not removed");
if ( (mu_cnt) == (1) )
{
mu_HomeNode.mu_state = mu_H_Invalid;
}
...
if ( !((mu_IsSharer( mu_msg.mu_src )) == (mu_true)) ) Error.Error("Assertion failed: AutoDowngradeReq from non-sharer");
if ( !(mu_false) ) Error.Error("Assertion failed: AutoDowngradeReq in shared state");
break;
default:
mu_ErrorUnhandledMsg ( mu_msg, (int)mu_HomeType );
break;
...
case mu_HT_Pending:
switch ((int) mu_msg.mu_mtype) {
case mu_DowngradeAck:
if ( !((mu_IsSharer( mu_msg.mu_src )) == (mu_true)) ) Error.Error("Assertion failed: DowngradeAck from non-sharer");
if ( !((mu_cnt) == (2)) ) Error.Error("Assertion failed: More than 2 sharers");
mu_HomeNode.mu_state = mu_H_Shared;
mu_HomeNode.mu_val = mu_msg.mu_val;
mu_Send ( mu_ReadAck, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val );
mu_SendAckToSharers ( mu_ReadAck, mu_HomeNode.mu_val, mu_msg.mu_src );
break;
case mu_AutoDowngradeReq:
bool mu__boolexpr18;
  if ((mu_cnt) == (0)) mu__boolexpr18 = TRUE ;
  else {
  mu__boolexpr18 = ((mu_IsSharer( mu_msg.mu_src )) == (mu_true)) ; 
}
if ( !(mu__boolexpr18) ) Error.Error("Assertion failed: DowngradeAck from non-sharer");
if ( mu_IsSharer( mu_msg.mu_src ) )
{
if ( !((mu_cnt) == (2)) ) Error.Error("Assertion failed: More than 2 sharers");
mu_HomeNode.mu_state = mu_H_Shared;
...
{
if ( !((mu_cnt) == (0)) ) Error.Error("Assertion failed: More than 0 sharers");
if ( !(!(mu_HomeNode.mu_owner.isundefined())) ) Error.Error("Assertion failed: owner undefined");
mu_HomeNode.mu_state = mu_H_Modified;
mu_HomeNode.mu_val = mu_msg.mu_val;
mu_Send ( mu_ReadExAck, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val );
}
break;
case mu_WBReq:
if ( (mu_cnt) == (0) )
...
{
if ( !(mu_HomeNode.mu_owner.isundefined()) ) Error.Error("Assertion failed: Owner is defined");
if ( !((mu_cnt) == (2)) ) Error.Error("Assertion failed: Sharers count is not 2");
if ( !((mu_IsSharer( mu_msg.mu_src )) == (mu_true)) ) Error.Error("Assertion failed: Writeback from non-sharer");
...
mu_HomeNode.mu_state = mu_H_Shared;
mu_HomeNode.mu_val = mu_msg.mu_val;
mu_SendAckToSharers ( mu_ReadAck, mu_msg.mu_val, mu_msg.mu_src );
mu_RemoveFromSharersList ( mu_msg.mu_src );
}
}
break;
...
break;
case mu_HT_S2MPending:
switch ((int) mu_msg.mu_mtype) {
case mu_WBReq:
if ( !((mu_HomeNode.mu_owner) != (mu_msg.mu_src)) ) Error.Error("Assertion failed: Writeback from owner");
if ( !((mu_IsSharer( mu_msg.mu_src )) == (mu_true)) ) Error.Error("Assertion failed: Writeback from non-sharer");
mu_RemoveFromSharersList ( mu_msg.mu_src );
if ( (mu_cnt) == (1) )
{
...
cout << "cnt = ";
mu_cnt.print();
if ( !((mu_IsSharer( mu_msg.mu_src )) == (mu_true)) ) Error.Error("Assertion failed: DowngradeReq from non-sharer");
if ( !(mu_false) ) Error.Error("Assertion failed: AutoDowngradeReq in HT_S2MPending state");
break;
default:
mu_ErrorUnhandledMsg ( mu_msg, (int)mu_HomeType );
break;
...
mu_ps = mu_P_Invalid;
break;
case mu_DowngradeReq:
mu_Send ( mu_DowngradeAck, mu_msg.mu_src, (int)mu_p, mu_VC1, mu_pv );
mu_ps = mu_PT_I2SPending;
break;
default:
...
break;
case mu_PT_M2SPending:
if ( !(!(mu_pv.isundefined())) ) Error.Error("Assertion failed: Error: Unexpected undefined value");
switch ((int) mu_msg.mu_mtype) {
case mu_ReadAck:
mu_ps = mu_P_Shared;
break;
...
public:
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_n.unionassign(r % 4);
    r = r / 4;
    while (what_rule < 28 && mu_midx.value()<4 )
      {
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("self-downgrade, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
  ...
  return (mu_p.mu_state) == (mu_P_Modified);
  }
    ...
    while (what_rule < 31 )
      {
        if ( ( TRUE  ) ) {
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
              if ((mu_p.mu_state) == (mu_P_Modified)) {
                if ( ( TRUE  ) )
                  return;
                else
                  what_rule++;
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_AutoDowngradeReq, (int)mu_HomeType, (int)mu_n, mu_VC2, mu_p.mu_val );
mu_p.mu_state = mu_PT_M2SPending;
  };
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("writeback from shared, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
  return (mu_p.mu_state) == (mu_P_Shared);
  }
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_WBReq, (int)mu_HomeType, (int)mu_n, mu_VC1, mu_1_Value_undefined_var );
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("writeback from modified, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
...
mu_p.mu_state = mu_PT_I2SPending;
  };
...
public:
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_ReadExReq, (int)mu_HomeType, (int)mu_n, mu_VC0, mu_1_Value_undefined_var );
mu_p.mu_state = mu_PT_S2MPending;
  };
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("read exclusive request, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
  ...
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_ReadExReq, (int)mu_HomeType, (int)mu_n, mu_VC0, mu_1_Value_undefined_var );
mu_p.mu_state = mu_PT_I2MPending;
  };
...
bool Condition(unsigned r)
{
  ...
  if (r>=43 && r<=45) return R7.Condition(r-43);
  if (r>=46 && r<=51) return R8.Condition(r-46);
...
void Code(unsigned r)
{
  ...
  if (r>=43 && r<=45) { R7.Code(r-43); return; } 
  if (r>=46 && r<=51) { R8.Code(r-46); return; } 
}
int Priority(unsigned short r)
{
  ...
  if (r>=43 && r<=45) { return R7.Priority(); } 
  if (r>=46 && r<=51) { return R8.Priority(); } 
}
char * Name(unsigned r)
{
  ...
  if (r>=43 && r<=45) return R7.Name(r-43);
  if (r>=46 && r<=51) return R8.Name(r-46);
  return NULL;
}
};
const unsigned numrules = 52;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 52
...
/********************
  Invariant records
 ********************/
int mu__invariant_19() // Invariant "Shared implies at least one sharer"
{
bool mu__boolexpr20;
  if (!((mu_HomeNode.mu_state) == (mu_H_Shared))) mu__boolexpr20 = TRUE ;
  else {
...
bool mu__condition_22() // Condition for Rule "Shared implies at least one sharer"
{
...
int mu__invariant_23() // Invariant "Modified implies only one modified processor"
{
bool mu__quant24; 
mu__quant24 = TRUE;
{
...
bool mu__boolexpr26;
  if (!((mu_HomeNode.mu_state) == (mu_H_Modified))) mu__boolexpr26 = TRUE ;
  else {
bool mu__boolexpr27;
  if ((mu_n1) == (mu_n2)) mu__boolexpr27 = TRUE ;
  else {
bool mu__boolexpr28;
  if ((mu_Procs[mu_n1].mu_state) != (mu_P_Modified)) mu__boolexpr28 = TRUE ;
  else {
  mu__boolexpr28 = ((mu_Procs[mu_n2].mu_state) != (mu_P_Modified)) ; 
}
...
bool mu__condition_29() // Condition for Rule "Modified implies only one modified processor"
{
...
int mu__invariant_30() // Invariant "Modified implies no shared processors"
{
bool mu__quant31; 
mu__quant31 = TRUE;
{
...
bool mu__boolexpr32;
  if (!((mu_HomeNode.mu_state) == (mu_H_Modified))) mu__boolexpr32 = TRUE ;
  else {
  mu__boolexpr32 = ((mu_Procs[mu_n].mu_state) != (mu_P_Shared)) ; 
}
if ( !(mu__boolexpr32) )
...
bool mu__condition_33() // Condition for Rule "Modified implies no shared processors"
{
...
int mu__invariant_34() // Invariant "Invalid implies no modified or shared processors"
{
bool mu__quant35; 
mu__quant35 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr36;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr36 = TRUE ;
  else {
bool mu__boolexpr37;
  if (!((mu_Procs[mu_n].mu_state) != (mu_P_Modified))) mu__boolexpr37 = FALSE ;
  else {
  mu__boolexpr37 = ((mu_Procs[mu_n].mu_state) != (mu_P_Shared)) ; 
}
...
bool mu__condition_38() // Condition for Rule "Invalid implies no modified or shared processors"
{
...
int mu__invariant_39() // Invariant "Shared implies no modified processors"
{
bool mu__quant40; 
mu__quant40 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr41;
  if (!((mu_HomeNode.mu_state) == (mu_H_Shared))) mu__boolexpr41 = TRUE ;
  else {
  mu__boolexpr41 = ((mu_Procs[mu_n].mu_state) != (mu_P_Modified)) ; 
}
if ( !(mu__boolexpr41) )
...
bool mu__condition_42() // Condition for Rule "Shared implies no modified processors"
{
...
int mu__invariant_43() // Invariant "Shared implies undifined owner"
{
bool mu__boolexpr44;
  if (!((mu_HomeNode.mu_state) == (mu_H_Shared))) mu__boolexpr44 = TRUE ;
  else {
  mu__boolexpr44 = (mu_HomeNode.mu_owner.isundefined()) ; 
}
return mu__boolexpr44;
};

bool mu__condition_45() // Condition for Rule "Shared implies undifined owner"
{
...
int mu__invariant_46() // Invariant "values in shared state match memory"
{
bool mu__quant47; 
mu__quant47 = TRUE;
{
...
bool mu__boolexpr48;
bool mu__boolexpr49;
  if (!((mu_HomeNode.mu_state) == (mu_H_Shared))) mu__boolexpr49 = FALSE ;
  else {
  mu__boolexpr49 = ((mu_Procs[mu_n].mu_state) == (mu_P_Shared)) ; 
}
  if (!(mu__boolexpr49)) mu__boolexpr48 = TRUE ;
  else {
  mu__boolexpr48 = ((mu_HomeNode.mu_val) == (mu_Procs[mu_n].mu_val)) ; 
}
if ( !(mu__boolexpr48) )
...
bool mu__condition_50() // Condition for Rule "values in shared state match memory"
{
...
int mu__invariant_51() // Invariant "values in memory matches value of last write, when shared or invalid"
{
bool mu__quant52; 
mu__quant52 = TRUE;
{
...
bool mu__boolexpr53;
bool mu__boolexpr54;
  if ((mu_HomeNode.mu_state) == (mu_H_Shared)) mu__boolexpr54 = TRUE ;
  else {
  mu__boolexpr54 = ((mu_HomeNode.mu_state) == (mu_H_Invalid)) ; 
}
...
bool mu__condition_55() // Condition for Rule "values in memory matches value of last write, when shared or invalid"
{
...
int mu__invariant_56() // Invariant "Invalid implies empty sharer list"
{
bool mu__boolexpr57;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr57 = TRUE ;
  else {
...
bool mu__condition_59() // Condition for Rule "Invalid implies empty sharer list"
{
...
int mu__invariant_60() // Invariant "modified implies empty sharers list"
{
bool mu__boolexpr61;
  if (!((mu_HomeNode.mu_state) == (mu_H_Modified))) mu__boolexpr61 = TRUE ;
  else {
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_HomeNode.mu_sharers.valid[(int)mu_i].value())
        {
          if ( mu_true ) mu__intexpr62++;
        }
      if (mu_i == 3-1) break;
    }
  }
/*** end multisetcount 6 declaration ***/
  mu__boolexpr61 = ((mu__intexpr62) == (0)) ; 
}
return mu__boolexpr61;
};

bool mu__condition_63() // Condition for Rule "modified implies empty sharers list"
{
...
int mu__invariant_64() // Invariant "value is undefined while invalid"
{
bool mu__quant65; 
mu__quant65 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr66;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_Invalid))) mu__boolexpr66 = TRUE ;
  else {
  mu__boolexpr66 = (mu_Procs[mu_n].mu_val.isundefined()) ; 
}
if ( !(mu__boolexpr66) )
...
bool mu__condition_67() // Condition for Rule "value is undefined while invalid"
{
...
int mu__invariant_68() // Invariant "values in valid state match last write"
{
bool mu__quant69; 
mu__quant69 = TRUE;
{
...
bool mu__boolexpr70;
bool mu__boolexpr71;
  if ((mu_Procs[mu_n].mu_state) == (mu_P_Shared)) mu__boolexpr71 = TRUE ;
  else {
  mu__boolexpr71 = ((mu_Procs[mu_n].mu_state) == (mu_P_Modified)) ; 
}
  if (!(mu__boolexpr71)) mu__boolexpr70 = TRUE ;
  else {
  mu__boolexpr70 = ((mu_Procs[mu_n].mu_val) == (mu_LastWrite)) ; 
}
if ( !(mu__boolexpr70) )
...
bool mu__condition_72() // Condition for Rule "values in valid state match last write"
{
...
int mu__invariant_73() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr74;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr74 = TRUE ;
  else {
...
bool mu__condition_75() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_76() // Invariant "Invalid implies empty owner"
{
bool mu__boolexpr77;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr77 = TRUE ;
  else {
...
bool mu__condition_78() // Condition for Rule "Invalid implies empty owner"
{
...
{"Invalid implies empty owner", &mu__condition_78, NULL, FALSE},
{"value in memory matches value of last write, when invalid", &mu__condition_75, NULL, FALSE},
{"values in valid state match last write", &mu__condition_72, NULL, FALSE},
{"value is undefined while invalid", &mu__condition_67, NULL, FALSE},
{"modified implies empty sharers list", &mu__condition_63, NULL, FALSE},
{"Invalid implies empty sharer list", &mu__condition_59, NULL, FALSE},
{"values in memory matches value of last write, when shared or invalid", &mu__condition_55, NULL, FALSE},
{"values in shared state match memory", &mu__condition_50, NULL, FALSE},
{"Shared implies undifined owner", &mu__condition_45, NULL, FALSE},
{"Shared implies no modified processors", &mu__condition_42, NULL, FALSE},
{"Invalid implies no modified or shared processors", &mu__condition_38, NULL, FALSE},
{"Modified implies no shared processors", &mu__condition_33, NULL, FALSE},
{"Modified implies only one modified processor", &mu__condition_29, NULL, FALSE},
{"Shared implies at least one sharer", &mu__condition_22, NULL, FALSE},
};
const unsigned short numinvariants = 14;
